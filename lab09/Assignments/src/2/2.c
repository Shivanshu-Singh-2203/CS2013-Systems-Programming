...
int delete(Node** ref_to_head, int num){
  ...
  // TODO: Read the docstring above and complete the code
	Node* elemPtr = search(*ref_to_head, num);
	if(elemPtr == NULL){
		return 0;
	}
	Node* temp = *ref_to_head;
	Node* nPtr = elemPtr->next;
	if(temp->data == num){
		if(nPtr)
		{nPtr->prev = NULL;}
		(*ref_to_head)->next = NULL;
		*ref_to_head = nPtr;	
		return 1;
	}


	while(temp->next != elemPtr){
		temp = temp->next;
	}
